...
function buildHtml({
  ...
  body = 'Toque no botao abaixo para entrar na sua conta. O link expira em 15 minutos e so pode ser usado uma vez.',
  buttonLabel = 'Entrar agora',
  footer = 'Voce esta recebendo este e-mail porque alguem solicitou um link de acesso para esta conta. Se nao foi voce, ignore esta mensagem.',
}) {
  return `<!DOCTYPE html>
<html>
          ...
          <table width="480" cellspacing="0" cellpadding="0" style="background:#1a1612;border-radius:16px;padding:40px;border:1px solid #2a221c;">
            <tr>
              <td align="center" style="padding-bottom:24px;">
                ...
                <h1 style="margin:0;color:#f5f1ea;font-size:22px;font-weight:800;letter-spacing:0.3px;">A. Coraca &amp; T. Carvalho</h1>
            ...
            <tr>
              <td style="padding:24px 0;border-top:1px solid #2a221c;">
                <h2 style="margin:0 0 12px;color:#f5f1ea;font-size:18px;font-weight:700;">${title}</h2>
                <p style="margin:0 0 24px;color:#a89a85;font-size:14px;line-height:1.6;">${body}</p>
                ...
                <p style="margin:24px 0 0;color:#6b5e4d;font-size:12px;line-height:1.6;word-break:break-all;">
                  Se o botao nao funcionar, copie este link no navegador:<br/>
            ...
            <tr>
              <td style="padding-top:24px;border-top:1px solid #2a221c;">
                <p style="margin:0;color:#6b5e4d;font-size:11px;line-height:1.6;">${footer}</p>
              </td>
            </tr>
          </table>
        </td>
...
exports.sendMagicLinkEmail = async ({
  to,
  link,
  subject = 'Seu link de acesso - A. Coraca & T. Carvalho',
  title,
  body,
  buttonLabel,
  footer,
  ...
  const result = await resend.emails.send({
    ...
    to,
    subject,
    html: buildHtml({ link, title, body, buttonLabel, footer }),
    text: text || `Seu link de acesso: ${link}`,
  });

  if (result?.error) {
    const err = new Error(`Resend rejected: ${result.error.message || JSON.stringify(result.error)}`);
    err.resendError = result.error;
    throw err;
  }

  console.log('[magic-link] e-mail aceito pelo Resend. id:', result?.data?.id);
  return result?.data;
};
